...
export default function XactScoreLogo({ compact = false }: { compact?: boolean }) {
  return (
    <div className={`flex items-center ${compact ? 'gap-2' : 'gap-3'}`}>
      ...
      <span className={`brand-wordmark whitespace-nowrap font-black leading-none tracking-[-0.06em] text-xactscore-text ${compact ? 'text-3xl' : 'text-4xl md:text-5xl'}`}>
        XactScore
      </span>
    </div>
  )
