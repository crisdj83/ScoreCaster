...
export default function ScoreCasterLogo({ compact = false }: { compact?: boolean }) {
  return (
    <div className={`flex items-center ${compact ? 'gap-2' : 'gap-3'}`}>
      <div className={`relative flex items-center justify-center rounded-2xl bg-[#101a35] text-[#d4ff00] shadow-lg ${compact ? 'h-9 w-9' : 'h-14 w-14'}`}>
      ...
      <span className={`font-black uppercase tracking-[-0.06em] text-[#101a35] ${compact ? 'text-lg' : 'text-4xl md:text-5xl'}`}>
        ScoreCaster
      </span>
    </div>
  )
