...
import { Target, BarChart2, BookOpen, Shield, Settings, CalendarDays } from 'lucide-react'
      ...
      <div className="flex items-center gap-1">
        {links.map(({ href, label, icon: Icon }) => {
          ...
          return (
            <Link
              ...
              title={label}
              className={cn(
                'flex min-h-11 flex-1 items-center justify-center gap-0.5 rounded-xl border py-2.5 outline-none transition-all duration-300 active:scale-95 focus-visible:outline-none sm:flex-row sm:gap-2 sm:py-2.5 sm:text-xs sm:tracking-wider md:px-4 md:text-sm',
        ...
        {isAdmin && (
          <Link
            ...
            title={t('Settings')}
            className={cn(
              'flex min-h-11 shrink-0 items-center justify-center gap-0.5 rounded-xl border px-3 py-2.5 outline-none transition-all duration-300 active:scale-95 focus-visible:outline-none sm:ml-auto sm:flex-row sm:gap-2 sm:py-2.5 sm:text-xs sm:tracking-wider md:px-4 md:text-sm',
