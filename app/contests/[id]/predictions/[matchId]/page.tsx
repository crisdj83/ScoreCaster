    ...
    <div className="mx-auto max-w-3xl space-y-6 pb-12">
      ...
      <div className="rounded-2xl bg-gray-950 p-6 text-white shadow-xl md:p-8">
        <div className="flex items-center gap-2 text-xs font-black uppercase tracking-widest text-[#d4ff00]">
          <Trophy className="h-4 w-4" /> Match predictions
        </div>
        ...
        <p className="mt-2 flex items-center gap-2 text-sm text-gray-300">
          <Clock className="h-4 w-4" /> {new Date(match.utcDate).toLocaleString()}
        </p>
      </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="font-black uppercase tracking-tight text-gray-900">
              {canReveal ? 'Predictions are now visible' : 'Predictions are hidden'}
            </p>
            <p className="text-sm text-gray-600">
              {canReveal ? 'Everyone’s submitted score is shown below.' : 'Scores unlock 30 minutes before kickoff.'}
            </p>
          </div>
        </div>
      </div>
      <div className="overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-lg">
        ...
        {players.length ? players.map(player => (
          ...
          <p className="p-8 text-center text-sm text-gray-500">No players are in this contest yet.</p>
        )}
      </div>
    </div>
  )
